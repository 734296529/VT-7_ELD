...
//256byte为一个Page
//4Kbytes(16个Page)为一个Sector
//16个扇区为1个Block
//W25Q256
//容量为32M字节,共有1024个Block,8192个Sector,131072个Page 
...
//	if(W25QXX_TYPE==W25Q256)				//如果SPI FLASH为W25Q256
//	{
...
//	}
	...
	for(i=0;i<5;i++)
	{
		if(W25QXX_ReadID()==W25Q256)				//如果SPI FLASH为W25Q256
			break;
		else if(i >= 4)
		{
	...
	for(i=0;i<5;i++)
	{
		...
		SPI1_ReadWriteByte(W25X_Enable4ByteAddr);//发送进入4字节地址模式指令   
		W25Qx_Disable();      			//取消片选 		
		if((W25QXX_ReadSR(3)&0X01) != 0)			//如果是4字节地址模式,
		{
			break;
		}	
		else if(i >= 4)
...
void W25QXX_Erase_Block(uint32_t Dst_Addr)
{
    ...
    W25QXX_Wait_Busy();   
  	W25Qx_Enable();                  			//使能器件   
    SPI1_ReadWriteByte(W25X_BlockErase);	//发送扇区擦除指令 
    if(W25QXX_TYPE==W25Q256)            	//如果是W25Q256的话地址为4字节的，要发送最高8位
    {
