...
/**
  * @brief  Writes page of data to the QSPI memory.
  * @param  pData: Pointer to data to be written
  * @param  WriteAddr: Write start address
  * @param  Size: Size of data to write,No more than 256byte.    
  * @retval QSPI memory status
...
uint8_t BSP_W25Qx_Page_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
	...
	/* Transmission of the data */
	if (HAL_SPI_Transmit(&hspi1, pData,Size, W25Qx_TIMEOUT_VALUE) != HAL_OK)
	{
		return W25Qx_ERROR;
	}
	W25Qx_Disable();
...
uint8_t BSP_W25Qx_Erase_Block(uint32_t Address)
{
	...
	{
		/* Check for the Timeout */
    if((HAL_GetTick() - tickstart) > W25Q128FV_BLOCK_ERASE_MAX_TIME)
    {        
			return W25Qx_TIMEOUT;
    }
	}
...
uint8_t BSP_W25Qx_Erase_Chip(void)
{
	...
	{
		/* Check for the Timeout */
    if((HAL_GetTick() - tickstart) > W25Q128FV_CHIP_ERASE_MAX_TIME)
    {        
			return W25Qx_TIMEOUT;
    }
	}
