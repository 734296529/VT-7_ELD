...
int setsetBaudRate(int deep)
{
	...
	work = data_len = ret = 0;
	writeCmd("atz\r");	//¸´Î»ÃüÁî£¬Ê¹¼Ä´æÆ÷ÉúĞ§
	while(work++ < request_cycle)
	{
		//ÊÕµ½Êı¾İ
		memset(cat,0,sizeof(cat));
...
int baudTest(void)
{
	...
	while(work++ < request_cycle)
	{
		...
		{
			if(strstr(cat,"not transmit"))//²¨ÌØÂÊ´íÎó
				return 0;
			else if(strstr(cat,"DATA"))		//²¨ÌØÂÊÕıÈ·
			{	
				return 1;
			}
			ret = 1;	
...
int backup_2Flash(void)
{
	...
	W25QXX_Write(write_buf[0],BackupAddr,sizeof(write_buf));	//´æ´¢4sµÄÊı¾İ
...
//	printf("[setSyncRange]:mStart=%d,mEnd=%d\r\n",mStart,mEnd);
	...
	u8 tmpbuf = 0;
	if(mStart<=mEnd || mStart>11520) //Í¬²½Ê±¼ä¶ÎºÏÀíĞÔÅĞ¶Ï
	{
...
//	printf("[sync_Handler]:0x%08x = 0x%02x\r\n",SyncAddrNow,tmpbuf[0][0]);
	...
	if(tmpbuf[0][0]==0x55 && tmpbuf[0][1]==0xFA)//ÓĞĞ§µÄÀúÊ·Êı¾İµØÖ·
	{
