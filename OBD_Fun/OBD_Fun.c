...
int DFL168_Init(void)
{
	...
	HAL_UART_Transmit(&huart2, (u8*)command, strlen(command), 0xFFFF);
	HAL_Delay(20);
	strcpy(command,"AT SP A\r");//Ñ¡Ôñ1939Ð­Òé
	HAL_UART_Transmit(&huart2, (u8*)command, strlen(command), 0xFFFF);	
	HAL_Delay(40);
...
int OBD_transData(u8* protocolData)
{
	...
	HAL_UART_Transmit(&huart3,protocolData,protocolData[2]+5,0xFFFF);//·¢ËÍÊý¾Ý
	while(tmp--)
	{
		if(recv_OK)//ÉÏÎ»»úÓ¦´ðÕýÈ·
		{
			recv_OK = 0;
			return 1;
		}
...
{
	memset(data_p,0,32);
//	if(!data_t.length)//¿ÕÊý¾Ý
//		return -1;
	data_p[0] = 0X55;		//Ð­ÒéÍ·
	data_p[1] = 0XFA;		//Ð­ÒéÍ·
	data_p[2] = data_t.length+2;	//³¤¶È£º²ÎÊý+Ö¸ÁîÂë
	data_p[3] = 0XD0;					//Ö¸ÁîÂë
	data_p[4] = data_t.type;	//Ö¸ÁîÂë
	memcpy(data_p+5,data_t.Data,data_t.length);		//²ÎÊý
	data_p[5+data_t.length] = getCRC8(data_p+3, data_p[2]);	//CRCÐ£Ñé
	data_p[6+data_t.length] = 0XEE;	//Ð­ÒéÎ²
	return 1;
}
/*
//Êý¾Ý±¸·ÝÖÁFlash
...
int OBD_sendBackup(void)
{
	...
	if(getCRC8(tmpbuf+3,tmpbuf[2]) != tmpbuf[2]+3)//Êý¾ÝÐ£Ñé³ö´í
	{
		SyncAddr += 16;		//Ìø¹ý±¾ÌõÊý¾Ý
		return 1;
	}
	
	OBD_transBackup(tmpbuf,SyncAddr);	//·¢ËÍÊý¾Ý
	SyncAddr += 16;
...
int OBD_transBackup(u8* data,u32 addr)
{
	...
	tmpbuf[0] = 0X55;		//Ð­ÒéÍ·
	tmpbuf[1] = 0XFA;		//Ð­ÒéÍ·
	tmpbuf[2] = data[2]+4;	//³¤¶È£º²ÎÊý+Ö¸ÁîÂë
	tmpbuf[3] = 0XB0;				//Ö¸ÁîÂë
	tmpbuf[4] = data[4];		//Ö¸ÁîÂë
	tmpbuf[5] = addr >> 24;	//²ÎÊý£ºFlashµØÖ·
	tmpbuf[6] = addr >> 16;
	tmpbuf[7] = addr >> 8;
	tmpbuf[8] = addr;
	memcpy(tmpbuf+9,data+5,data[2]-2);		//²ÎÊý:Öµ+Ê±¼ä
	tmpbuf[3+tmpbuf[2]] = getCRC8(tmpbuf+3, tmpbuf[2]);	//CRCÐ£ÑéÖµ
	tmpbuf[4+tmpbuf[2]] = 0XEE;	//Ð­ÒéÎ²
...
*/	
	...
	if(getSpaceNum(cat)>=7) //ÅÐ¶Ï¿Õ¸ñÊýÁ¿
	{
		u32 a = htoi(cat[6]);
		u32 b = htoi(cat[7]);
		sub = a * 16 + b;		
...
//	}
	...
	if(sub > 0){//×ªËÙ²»Îª0
		isStartUp = 1;		//ÐÞ¸Ä·¢¶¯»ú×´Ì¬Îªµã»ð
		Sleep_Stop();			//Í£Ö¹ÐÝÃß¼ÆÊ±
	}
	else if(sub <= 0 && !SyncFlag){//×ªËÙÎª0,·ÇÍ¬²½×´Ì¬,ÇÒ·¢¶¯»ú×´Ì¬Îªµã»ð
		if(isStartUp == 1){
			Sleep_Start();
		}
		isStartUp = 0;		//ÐÞ¸Ä·¢¶¯»ú×´Ì¬ÎªÏ¨»ð
	}
...
int getEngineHours(void) 
{
	...
	if(getSpaceNum(cat)>=7)//ÅÐ¶Ï¿Õ¸ñÊýÁ¿
	{
		...
		sub = (a +b*256 + c*65536 + d*16777216)/20;//ÒýÇæÊ±¼ä
	}
...
*/	
	...
	if(getSpaceNum(cat)>=7)//¼ÆËã¿Õ¸ñÊýÁ¿
	{
		u32 a = charhextoascii(cat);
		u32 b = charhextoascii(cat+3);
		u32 c = charhextoascii(cat+6);
...
int protHandler(u8* recvCmd,u8 len)
{
	...
	if(ProtState ==1 && recvCmd[len-1] == 0XEE)
	{
		...
		if(getCRC8(ProtRecvBuff+3,ProtRecvBuff[2]) == ProtRecvBuff[ProtRecvLen-2]){
			ProtState = 2;//Êý¾ÝÐ£Ñé³É¹¦£¬¿É½øÐÐ½âÎö
		}
		else{
	...
	switch(CodeNum)
	{
		...
		case 0X0004://ÉèÖÃÐÝÃßÊ±¼ä
		{
			...
			break;
		case 0X0005://ÉèÖÃ¶ÁÈ¡Êý¾ÝÀàÐÍ
		{
		}
			break;
		case 0X5001://ÇëÇó½øÐÐOTAÉý¼¶
		{
			iap_load_boot();
		}			
			break;
...
u8 readCmd(u8* Data,u8* data_len,u32 timeout)
{
	...
	{
		if(recv_end_flag2)//½ÓÊÕµ½Êý¾Ý
		{	
			{
				memcpy(Data, ReceiveBuff2, Rx_len2); //¿½±´´®¿Ú½ÓÊÕµ½µÄÊý¾Ý
				*data_len = Rx_len2;               //¼ÇÂ¼Êý¾Ý³¤¶È
			}
			/*	Çå¿Õ½ÓÊÕ»º´æÇø	*/
			memset(ReceiveBuff2,0,sizeof(ReceiveBuff2));
			/*	½ÓÊÕÊý¾Ý³¤¶ÈÇåÁã	*/
...
int setSleepDelay(u8 slpDly)
{
	...
	//¼ÆËãÏ¨»ðÐÝÃßÑÓÊ±£¬µ¥Î»ÓÉmin×ª»»Îªs
	u8 tmpbuf[4] = {0};
	tmpbuf[0] = sleepDelay >> 24;
	tmpbuf[1] = sleepDelay >> 16;
	tmpbuf[2] = sleepDelay >> 8;
	tmpbuf[3] = sleepDelay;
	W25QXX_Write(tmpbuf,FLASH_ADDR(8191,1),4);	//´¢´æÐÝÃßÊ±¼äÉèÖÃ
	sleepCount = 0;
	return 1;
}
...
int backup_2Flash(void)
{
	...
	if(BackupAddr % SECTOR_SIZE ==0) 		//µØÖ·½øÈëÐÂµÄÉÈÇøÊ±
	{
		W25QXX_Erase_Sector(BackupAddr);	//ÏÈ²Á³ýÉÈÇø
	}
	W25QXX_Write_NoCheck(write_buf[0],BackupAddr,sizeof(write_buf));	//´æ´¢4sµÄÊý¾Ý

	BackupAddr += sizeof(write_buf);		//´æ´¢µØÖ·Æ«ÒÆ	
	if(BackupAddr >= OBD_FLASH_END)			//¿ØÖÆµØÖ·ÔÚÏÞ¶¨·¶Î§ÄÚ
	{
		BackupAddr = OBD_FLASH_START; 		//±¸·ÝµØÖ··µ»ØÊ×µØÖ·
	}
	...
	if((AddrIndex+8) >= ADDR_INDEX_END)			//¿ØÖÆË÷ÒýÇøÔÚÏÞ¶¨·¶Î§ÄÚ
	{
		AddrIndex = ADDR_INDEX_START; 				//Ë÷ÒýÇøµØÖ··µ»ØÊ×µØÖ·	
		W25QXX_Erase_Sector(FLASH_ADDR(8190,0));		//ÏÈ²Á³ýË÷ÒýÇø
		W25QXX_Write_NoCheck(tmpbuf, AddrIndex, 8);	//¸üÐÂË÷ÒýÇøÄÚÈÝ
	}	
	else
	{
		AddrIndex += 8;
		W25QXX_Write(tmpbuf, AddrIndex, 8);	//¸üÐÂË÷ÒýÇøÄÚÈÝ
		
		tmpbuf[0] = 0x00;
		W25QXX_Write_NoCheck(tmpbuf,(AddrIndex-8), 1);	//¾ÉË÷ÒýÇø±ê¼Ç
	}
...
/*
	u8 tmpbuf[0] = 0x00;
	W25QXX_Write_NoCheck(tmpbuf, AddrIndex, 1);	//¾ÉË÷ÒýÇø±ê¼Ç
	
	AddrIndex += 8;											//Ë÷ÒýÇøÆ«ÒÆ
	if(AddrIndex >= ADDR_INDEX_END)			//¿ØÖÆË÷ÒýÇøÔÚÏÞ¶¨·¶Î§ÄÚ
	{
		AddrIndex = ADDR_INDEX_START; 		//Ë÷ÒýÇøµØÖ··µ»ØÊ×µØÖ·
		W25QXX_Erase_Sector(FLASH_ADDR(8190,0));	//ÏÈ²Á³ýË÷ÒýÇø
	}
...
//	printf("[setSyncRange]:mStart=%d,mEnd=%d\r\n",mStart,mEnd);
	...
	if(mStart<=mEnd || mStart>11520 || mEnd>11520) //Í¬²½Ê±¼ä¶ÎºÏÀíÐÔÅÐ¶Ï
	{
//		printf("[setSyncRange] fail 0\r\n");
	...
	if(tmpbuf[0][0]==tmpbuf[1][0] && tmpbuf[0][0] == 0xb0)//Ê±¼ä·¶Î§ÄÚ¶¼ÓÐÐ§
	{
		SyncFlag = 1;
		return 1;
	}
	
	if(tmpbuf[1][0]!=0xb0 && mEnd!=0)//¸ÃÊ±¼ä·¶Î§ÄÚÎÞÊý¾Ý
	{
...
//	printf("[sync_Handler]:0x%08x = 0x%02x\r\n",SyncAddrNow,tmpbuf[0][0]);
	...
	if(tmpbuf[0][0]==0xb0)//ÓÐÐ§µÄÀúÊ·Êý¾ÝµØÖ·
	{
...
void sync_SendData(u8 mbuf[][32])
{
	...
	tmpbuf[3] = 0XB0;		//Ö¸ÁîÂë
	tmpbuf[4] = 0x00;		//ÀúÊ·Êý¾Ý
	tmpbuf[25] = 0xEE;	//Ð­ÒéÎ²	

	for(i=0;i<4;i++)
	{				
		memcpy(tmpbuf+5,&(mbuf[i][1]),19);	//±¸·ÝÊý¾Ý
